...
class OrderController extends Controller
{
        ...
        foreach ($request['cart'] as $c) {
            if ($c['item_campaign_id'] != null) {
                ...
                if ($product) {
                    ...
                    $fixed = $price * $c['quantity'];
                    $cat = Category::find($product['category_id']);
                    $calc = $fixed * ($cat->commission /100);
                    $commission = number_format((float)$calc, 2, '.', '');
    ...
    public function place_order_subscribe(Request $request)
    {
        ...
        // $restaurant = Restaurant::with('discount')->selectRaw('*, IF(((select count(*) from `restaurant_schedule` where `restaurants`.`id` = `restaurant_schedule`.`restaurant_id`  > 0), true, false) as open')->where('id', $request->restaurant_id)->first();
        ...
        $subscription->vendor_id = $request->restaurant_id;
        $subscription->customer_id = auth('api')->user()->id;
        $subscription->address = json_encode($address);
        $subscription->days = json_encode($request->days);
        $subscription->product_id = $request->productId;
        $subscription->price = $request->order_amount;
        $subscription->schedule_at = $request->schedule_at;
        ...
        // try{
            ...
            $single_amount = floatval($request->order_amount / count($request->selectedDate));
            
            foreach($request->selectedDate as $current){
                \DB::table('subscription_repeats')->insert([
                    'subscription_id' => $subscription->id,
                    'order_date' => $current,
                    'price' => number_format($single_amount, 2, '.', ''),
                    'status' => 'Pending'
                    ]);
            }
             ...
             return response()->json([
                ...
                'total_ammount' => $request->order_amount
            ], 200);
        // }
        // catch(\Exception $ex){
        //     \Log::info($ex);
    ...
    //       return response()->json([$myErr], 403);
    //   }
    }
