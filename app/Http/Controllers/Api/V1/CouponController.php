...
class CouponController extends Controller
{
    public function list(Request $request)
    {
        ...
        $data = [];
        // try {
            $coupons = Coupon::active()->whereDate('expire_date', '>=', date('Y-m-d'))->whereDate('start_date', '<=', date('Y-m-d'))->where('subscribe_type',0)->get();
    ...
    public function subscription(Request $request)
    {
        ...
        $data = [];
        // try {
            $coupons = Coupon::active()->whereDate('expire_date', '>=', date('Y-m-d'))->whereDate('start_date', '<=', date('Y-m-d'))->where('subscribe_type',1)->get();
            foreach($coupons as $key=>$coupon)
            {
                if($coupon->coupon_type == 'restaurant_wise')
                {
